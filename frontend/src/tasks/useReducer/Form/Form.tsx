...
export default function Form(): ReactElement {
    ...
    return <form>
        <label>name
            <input type="text" minLength={1} value={user.name} onChange={(e) => userDispatcher({ type: 'NAME', name: e.target.value })} />
        </label>
        <label>email
            <input type="text" minLength={1} value={user.email} onChange={(e) => userDispatcher({ type: 'EMAIL', email: e.target.value })} />
        </label>
    </form >
}